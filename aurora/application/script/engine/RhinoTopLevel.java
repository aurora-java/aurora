 ...
 * functions (bindings, scope, sync) and two constructors (JSAdapter,
 * JavaAdapter).
 * 
...
public final class RhinoTopLevel extends ImporterTopLevel {
	...
	RhinoTopLevel(Context cx, RhinoScriptEngine engine) {
		 ...
		 * initializer attempts to define a new property. But, JavaAdapter is an
	...
	/**
	 * The bindings function takes a JavaScript scope object of type
	 * ExternalScriptable and returns the underlying Bindings instance.
	 * 
	 * var page = scope(pageBindings); with (page) { // code that uses page
	 * scope } var b = bindings(page); // operate on bindings here.
	 */
	...
	/**
	 * The scope function creates a new JavaScript scope object with given
	 * Bindings object as backing store. This can be used to create a script
	 * scope based on arbitrary Bindings instance. For example, in webapp
	 * scenario, a 'page' level Bindings instance may be wrapped as a scope and
	 * code can be run in JavaScripe 'with' statement:
	 * 
	 * var page = scope(pageBindings); with (page) { // code that uses page
	 * scope }
	 */
	 ...
	 * thread(function() {o.f(5);}); entry js> thread(function() {o.f(5);}); js>
	 * exit entry exit
	 */
