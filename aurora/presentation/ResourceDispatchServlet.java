...
public class ResourceDispatchServlet extends HttpServlet implements IResourceUrlMapper {
    ...
    private static void parseRequestLine( String root_path, String uri, StringBuffer package_name, StringBuffer theme_name, StringBuffer resource )
    {
        ...
        int end_id = uri.indexOf('/', begin_id+1);
        if(begin_id<0||end_id<0) throw new IllegalArgumentException();
        if(begin_id>=end_id) throw new IllegalArgumentException();
        begin_id++;
        setString(package_name, uri.substring(begin_id, end_id));
        
        String file_name = uri.substring( end_id + 1 );
        int theme_end_id = file_name.indexOf('/');
        ...
        throws ServletException, IOException 
    {
        ...
        try{
            ...
            ViewComponentPackage pkg = mPresentationManager.getPackage(package_name.toString());
            if(pkg==null) throw new IllegalArgumentException();
            File rf = pkg.getResourceFile(theme.toString(), resource.toString());
            if(rf==null) throw new IllegalArgumentException();
    ...
    public void writeResponseFile( File resource, HttpServletResponse response )
        throws IOException
    {
        String mime_type = mServletContext.getMimeType(resource.getPath());
        if(mime_type!=null)
    ...
    public static void main(String[] args) throws Exception {
        String root_path = "/resource";
        String uri = "/resource/my.pkg/default/css/a.css";
        StringBuffer package_name = new StringBuffer();
        StringBuffer theme = new StringBuffer();
        StringBuffer resource = new StringBuffer();
        parseRequestLine(root_path, uri, package_name, theme, resource);
