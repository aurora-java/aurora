		...
		if (tabs != null) {
			...
			if (childs != null) {
				...
				while (it.hasNext()) {
					...
					//调整strips的宽度，在ie的兼容模式下，会出现换行的问题
					stripswidth += width + 16;
					String id = tc.getId("");
					...
					if (!"".equals(target)) {
						...
						for (int b = 0; b < ts.length; b++) {
							String tid = ts[b];
							st.append("$au('" + tid
									+ "').on('valid',function(ds, record, name, valid){if(!valid && !Ext.get('" + id
									+ "').hasActiveFx()) Ext.get('" + id
					...
					sb.append("' " + tabStyle + " unselectable='on' " + ((!"".equals(id)) ? "id='" + id + "'" : "")
