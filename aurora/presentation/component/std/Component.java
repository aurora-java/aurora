...
public class Component {
    ...
    public void onPreparePageContent(BuildSession session, ViewContext context)
        ...
        addJavaScript(session, context,
                "locale/aurora-lang-" + session.getLanguage() + ".js");
    ...
    protected Integer getComponentWidth(CompositeMap model, CompositeMap view,
            Map map) {
        ...
        String widthStr = view.getString(ComponentConfig.PROPERTITY_WIDTH, ""
                + getDefaultWidth());
        String wstr = uncertain.composite.TextParser.parse(widthStr, model);
        Integer width = "".equals(wstr) ? new Integer(getDefaultWidth())
                : Integer.valueOf(wstr);
        ...
        if (marginWidth != null && vw != null) {
            width = new Integer(
                    (vw.intValue() - marginWidth.intValue()) > width.intValue() ? (vw
                            .intValue() - marginWidth.intValue()) : width
                            .intValue());
    ...
    protected Integer getComponentHeight(CompositeMap model, CompositeMap view,
            Map map) {
        CompositeMap root = view.getRoot();
        ...
        String heightStr = view.getString(ComponentConfig.PROPERTITY_HEIGHT, ""
                + getDefaultHeight());
        String hstr = uncertain.composite.TextParser.parse(heightStr, model);
        Integer height = "".equals(hstr) ? new Integer(getDefaultHeight())
                : Integer.valueOf(hstr);
        Integer marginHeight = view.getInt(PROPERTITY_MARGIN_HEIGHT);
        if (marginHeight != null && vh != null) {
            height = new Integer(
                    (vh.intValue() - marginHeight.intValue()) > height
                            .intValue() ? (vh.intValue() - marginHeight
    ...
    public void onCreateViewContent(BuildSession session, ViewContext context)
        ...
        /** 绑定DataSet **/
        String bindTarget = view.getString(
                ComponentConfig.PROPERTITY_BINDTARGET, "");
     ...
     * @return String
     */
    protected void addJavaScript(BuildSession session, ViewContext context,
            String javascript) {
     ...
     * @return String
     */
    protected void addStyleSheet(BuildSession session, ViewContext context,
            String style) {
    ...
    protected String getFieldPrompt(BuildSession session, CompositeMap field,
            String dataset) {
        ...
        if ("".equals(label)) {
            ...
            if (ds != null) {
                CompositeMap fieldcm = ds
                        .getChild(DataSetConfig.PROPERTITY_FIELDS);
                if (fieldcm != null) {
                    List fields = fieldcm.getChilds();
                    Iterator it = fields.iterator();
                    while (it.hasNext()) {
                        CompositeMap fieldMap = (CompositeMap) it.next();
                        String fn = fieldMap.getString(
                                ComponentConfig.PROPERTITY_NAME, "");
                        if (name.equals(fn)) {
                            label = fieldMap.getString(
                                    ComponentConfig.PROPERTITY_PROMPT, "");
                            break;
                        }
                    }
                }
    ...
    private CompositeMap getDataSet(BuildSession session, String dataSetName) {
        ...
        ServiceInstance svc = (ServiceInstance) session
                .getInstanceOfType(IService.class);
