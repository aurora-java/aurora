...
/**
 * Build an existing screen at current position <code>
...
public class ScreenInclude implements IViewBuilder, ISingleton {
	...
	public static CompositeMap createScreenIncludeConfig(String screen_name) {
		CompositeMap config = new CompositeMap("a",
				Namespace.AURORA_FRAMEWORK_NAMESPACE, "screen-include");
	...
	public ScreenInclude(IObjectRegistry reg) {
		HttpServiceFactory fact = (HttpServiceFactory) reg
				.getInstanceOfType(HttpServiceFactory.class);
		IProcedureManager pm = (IProcedureManager) reg
				.getInstanceOfType(IProcedureManager.class);
		IApplicationConfig config = (IApplicationConfig) reg
	...
	private void init(HttpServiceFactory fact, IProcedureManager pm,
			IApplicationConfig config) {
		...
		if (mApplicationConfig != null) {
			...
			if (view_config != null) {
				...
				mDefaultIncludedScreenTemplate = view_config
						.getString(DEFAULT_INCLUDED_SCREEN_TEMPLATE);
	...
	public HttpServiceInstance createSubInstance(String name,
			CompositeMap context) throws SAXException, IOException {
		// CompositeMap context = view_context.getModel().getRoot();
		HttpServiceInstance parent = (HttpServiceInstance) ServiceInstance
				.getInstance(context);
		...
		svc.getController().setProcedureName(
				ControllerProcedures.RUN_INCLUDED_SCREEN);
		return svc;
	}

	public void doScreenInclude(BuildSession session, CompositeMap view,
			CompositeMap root) throws Exception {
		String screen_name = view.getString(KEY_SCREEN);
		if (screen_name == null)
			throw new ConfigurationError(
					"'screen' property must be set for <screen-include>");
		...
		// Run service
		try {
			HttpServiceInstance sub_instance = createSubInstance(screen_name,
					root);
			ServiceInstance.setInstance(root, sub_instance);
			Procedure proc = AbstractFacadeServlet.getProcedureToRun(
					mProcedureManager, sub_instance);
	...
	public void buildView(BuildSession session, ViewContext view_context)
		...
		try {
			doScreenInclude(session, view_context.getView(), view_context
					.getModel().getRoot());
		} catch (Exception ex) {
			throw new ViewCreationException(
					"Error when invoking screen config file +" + view.toXML(),
					ex);
		}
	}
