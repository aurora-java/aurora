...
import aurora.security.crypto.util.EncodingUtils;
...

{
	...
	public AesBytesEncryptor(String password, CharSequence salt, int keysize) {
	...
	public AesBytesEncryptor(String password, CharSequence salt,
			BytesKeyGenerator ivGenerator, int keysize) {
		PBEKeySpec keySpec = new PBEKeySpec(password.toCharArray(),
				Hex.decode(salt), 1024, keysize);
		...
		this.ivGenerator = (ivGenerator != null ? ivGenerator
				: NULL_IV_GENERATOR);
	}
	...
	public byte[] encrypt(byte[] bytes) {
		synchronized (this.encryptor) {
			...
			CipherUtils.initCipher(this.encryptor, Cipher.ENCRYPT_MODE,
					this.secretKey, new IvParameterSpec(iv));
			byte[] encrypted = CipherUtils.doFinal(this.encryptor, bytes);
			return this.ivGenerator != NULL_IV_GENERATOR ? EncodingUtils
					.concatenate(new byte[][] { iv, encrypted }) : encrypted;
		}
	}
	...
	public byte[] decrypt(byte[] encryptedBytes) {
		synchronized (this.decryptor) {
			...
			CipherUtils.initCipher(this.decryptor, Cipher.DECRYPT_MODE,
					this.secretKey, new IvParameterSpec(iv));
			return CipherUtils.doFinal(
					this.decryptor,
					this.ivGenerator != NULL_IV_GENERATOR ? encrypted(
							encryptedBytes, iv.length) : encryptedBytes);
		}
	}

	private byte[] iv(byte[] encrypted) {
		return this.ivGenerator != NULL_IV_GENERATOR ? EncodingUtils.subArray(
				encrypted, 0, this.ivGenerator.getKeyLength())
				: NULL_IV_GENERATOR.generateKey();
	}

	private byte[] encrypted(byte[] encryptedBytes, int ivLength) {
		return EncodingUtils.subArray(encryptedBytes, ivLength,
