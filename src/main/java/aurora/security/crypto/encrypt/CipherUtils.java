...
class CipherUtils {
	...
	public static <T extends AlgorithmParameterSpec> T getParameterSpec(
			Cipher cipher, Class<T> parameterSpecClass) {
		try {
	...
	public static void initCipher(Cipher cipher, int mode, SecretKey secretKey,
		...
		initCipher(cipher, mode, secretKey, new PBEParameterSpec(salt,
				iterationCount));
	}
	...
	public static byte[] doFinal(Cipher cipher, byte[] input) {
		try {
			...
			throw new IllegalStateException(
					"Unable to invoke Cipher due to bad padding", e);
		}
	}
}